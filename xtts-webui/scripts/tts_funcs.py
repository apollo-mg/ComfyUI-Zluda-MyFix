...
class TTSWrapper:
    def __init__(self, output_folder="./output", speaker_folder="./speakers", lowvram=False, model_source="local", model_version="2.0.2", device="cuda"):
        ...
        self.cuda = device
        # ZLUDA Final Fix: Force CPU execution because ZLUDA/ROCm fails on both FP32 and BF16 GEMM kernels for this model.
        self.device = "cpu" 
    ...
    def get_or_create_latents(self, speaker_name, speaker_wav):
        ...
        if speaker_name not in self.latents_cache:
            ...
            # ZLUDA Fix: Ensure model is on CPU for this operation to avoid cuFFT errors
            ...
            # Ensure latents are on the target device (self.device)
            target_device = self.device
            if target_device == 'cpu' and torch.cuda.is_available() and self.cuda != 'cpu':
                 # If lowvram is on, self.device might be cpu, but we might want them on gpu when used?
                 # Actually, inference moves tensors as needed. But keeping them on CPU is safe.
                 pass
    ...
    def local_generation(self,this_dir, text, ref_speaker_wav, speaker_wav, language, options, output_file,is_inbuild = False):
        ...
        # ZLUDA Fix: Use autocast to bfloat16 to avoid cublasSgemm float32 crashes
        # while allowing sensitive layers like LayerNorm to fall back to float32 safely.
