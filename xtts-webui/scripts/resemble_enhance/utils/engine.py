...
try:
    ...
    DeepSpeedEngine = nn.Module  # Dummy base class
    ...
    def __init__(self, *args, ckpt_dir, **kwargs):
        ...
        if deepspeed:
            super().__init__(args=None, *args, **kwargs)
             ...
             # Fallback for No DeepSpeed
            nn.Module.__init__(self)
            self.module = kwargs.get("model")
            # We need to register the module so it's part of the state dict
            if self.module:
                self.add_module("module", self.module)
            self.mpu = None
